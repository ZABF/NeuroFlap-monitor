...
class PlotWindow(QWidget):
    def __init__(self):
        ...
        signal_source_layout.addWidget(QLabel("ESP32 Signal Export (Dynamic):"))
        signal_source_layout.addWidget(self.active_source_label)
        signal_source_layout.addStretch()
        neuroflap_page_layout.addLayout(signal_source_layout)
    ...
    def activate_live_signal_export_descriptors(self, descriptors, host, port):
    ...
    def _remove_empty_signal_export_section(self, section):
        info = self.signal_export_sections.get(section)
        ...
        if box is not None:
            ...
            box.deleteLater()
        self.signal_export_sections.pop(section, None)
        if section in self.signal_export_section_order:
            self.signal_export_section_order.remove(section)
    ...
    def _remove_dynamic_raw_variable(self, var_name):
        ...
        if info and var_name in info.get("items", []):
            ...
            self._relayout_signal_export_section_items(section)
            self._remove_empty_signal_export_section(section)
    ...
    def register_signal_export_descriptors(self, descriptors):
        ...
        for desc in normalized:
            ...
            if last_section:
                ...
                if last_info and var_name in last_info.get("items", []):
                    ...
                    self._remove_empty_signal_export_section(last_section)
        ...
        self.signal_export_count = len(display_names)
