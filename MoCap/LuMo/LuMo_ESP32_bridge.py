...
# UDP transmit
esp32_ip="192.168.236.44"
esp32_port=28090
udp_sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)

def send_udp_message(rigid_data):
    frame_header=0xBB
    if rigid_data.Name=="body":
        func_code=0x01
    elif rigid_data.Name=="wing":
        func_code=0x02
    payload =[
        int(rigid_data.Id),
    ...
    # 发送数据
    udp_sock.sendto(packed_data, (esp32_ip, esp32_port))
...
try:
    ...
    while True:
        ...
        # uCameraSyncTime = time.time_ns()/1000
        print("uCamera")
        print(uCameraSyncTime) #打印相机同步时间
        uBroadcastTime = frame.uBroadcastTime
        # uBroadcastTime = time.time_ns()/1000
        print("uBroadcast")
        ...
        for rigid in frame.rigidBodys:
            if rigid.IsTrack is True: #判断刚体追踪状态
                ...
                # send_udp_message(rigid) #传递数据给esp32
                row_data = frame_data.copy()  # 复制基本数据
                    # 添加刚体数据
                row_data.extend([
                    None,
